...
class GtfsTripMatcherTest {
  ...
  private static final ServiceTime START_TIME = ServiceTime.parse("10:00:00");
  ...
  void rejectsAStartTimeNoTripDepartsAt() {
    assertNoMatch(
      reference().withStartTime(ServiceTime.parse("11:11:11")).withDirection(Direction.OUTBOUND)
    );
  }
  ...
  void rejectsAReferenceThatNamesNoTripAtAll() {
    var reference = TripReference.builder()
      .withRouteId(route.getId())
      .withStartTime(ServiceTime.parse("11:11:11"))
      .withStartDate(SERVICE_DATE)
      .withDirection(Direction.OUTBOUND);
  ...
  void matchesATripCarriedOverFromThePreviousDay() {
    var match = match(
      reference().withStartTime(ServiceTime.parse("01:30:00")).withDirection(Direction.OUTBOUND)
    );
