...
class AbsoluteTimeUpdateTest {
  ...
  private static final ServiceTime AIMED_TIME = ServiceTime.ofSecondsPastMidnight(3600);
  // 1 hour 5 minutes after midnight
  private static final ServiceTime ACTUAL_TIME = ServiceTime.ofSecondsPastMidnight(3900);
  ...
  void resolveTimeIgnoresScheduledTime() {
    ...
    assertThat(update.resolveTime(AIMED_TIME.secondsPastMidnight())).isEqualTo(
      ACTUAL_TIME.secondsPastMidnight()
    );
    assertThat(update.resolveTime(0)).isEqualTo(ACTUAL_TIME.secondsPastMidnight());
  }
