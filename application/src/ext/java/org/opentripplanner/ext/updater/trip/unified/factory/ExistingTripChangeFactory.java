...
import org.opentripplanner.ext.updater.trip.unified.model.change.TripModification;
...
public class ExistingTripChangeFactory {
   ...
   * {@link org.opentripplanner.ext.updater.trip.unified.service.TripReviser TripReviser}.
   *
  ...
  /**
   * Create the modification of the stop pattern of an existing trip, for the {@link org.opentripplanner.ext.updater.trip.unified.service.TripModifier TripModifier}.
   *
  ...
  private List<ResolvedStopTimeUpdate> resolveStopTimeUpdates(
    ...
    return updates
      .stream()
      .map(u ->
        ResolvedStopTimeUpdate.of(u, serviceDate, timeZone, resolveStops(u.stopReference()))
      )
      .toList();
  }

  /** Look up the stop the call is at and the stop assigned in its place, each on its own. */
  private ResolvedStopReference resolveStops(StopReference stopReference) {
    return new ResolvedStopReference(
      stopResolver.resolveReferencedStop(stopReference),
      stopResolver.resolveAssignedStop(stopReference)
    );
  }
