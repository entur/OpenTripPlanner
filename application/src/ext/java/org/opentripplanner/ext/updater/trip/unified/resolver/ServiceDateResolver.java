...
public class ServiceDateResolver {
   ...
   * Resolution order:
   * <ol>
   *   <li>If serviceDate is present in command, return it</li>
   ...
   */
  public LocalDate resolveServiceDate(TripUpdateCommand command) {
    var serviceDate = command.serviceDate();
    ...
    var aimedDepartureTime = command.aimedDepartureTime();
    if (aimedDepartureTime != null) {
      return resolveFromAimedDepartureTime(command, aimedDepartureTime);
    }
   ...
   * the departure falls one day past the service date, so the offset is 1. For trips departing
   * before midnight on a previous day (rare), the offset is negative.
   *
  ...
  private int calculateDayOffset(Trip trip) {
    ...
    return ServiceTime.ofSecondsPastMidnight(tripTimes.getDepartureTime(0)).dayOffset();
  }
}
