...
public final class TripCreation extends TripAddition {
  ...
  /**
   * The submode classifying the created trip, derived from the line it is classified against. Null
   * when nothing classifies it - the trip then takes the submode of {@link #route}.
   */
  @Nullable
  ...
  /**
   * The operator the created trip is operated by, resolved against the transit model. Null when the
   * message names none - the trip then takes the operator of {@link #route}.
   */
  @Nullable
  ...
  public TripCreation(
    ...
    this.tripShortName = creationInfo.tripShortName();
    validate();
  }
   ...
   * @param patternLookup      finds the real-time pattern the created trip's stop pattern maps to,
   *                           creating it if this is the first trip to run it
  ...
  public TripUpdateResult apply(
    ...
    if (includeScheduledData) {
      // SIRI-style: the aimed times of the created trip live in the scheduled timetable of its
      // pattern, so the trip cannot share a pattern with other added trips - it gets one of its own.
      ...
      // GTFS-RT style: the pattern carries no scheduled times, so every added trip running this
      // stop pattern shares one real-time pattern - a corridor served by many replacement trips
      // gets one pattern rather than one per trip. The created trip is not in the transit model
      // yet, so it has no scheduled pattern to fall back to.
      pattern = patternLookup.findOrCreate(stopTimesAndPattern.stopPattern(), trip, null);
    }
    ...
    builder.withAdded();
    if (isCancelled(stopTimesAndPattern.stopPattern())) {
    ...
    // tripCreation=true since we're creating a new trip. Only a format that rebuilds an added trip
    // on every message reverts: it builds the trip here again and has to take it off whatever
    // pattern the previous message put it on. A format that adds a journey once has nothing to
    // revert, and legacy says so outright - `revert = !tripCreation` in SiriRealTimeUpdateHandler.
    var realTimeTripUpdate = RealTimeTripUpdate.of(pattern, builder.build(), serviceDate())
      ...
      .withRevertPreviousRealTimeUpdates(isRebuiltFromEveryMessage())
  ...
  /**
   * Apply the mode and short name the message describes the created trip with, and the operator and
   * submode resolved for it. Falls back to the mode of {@link #route} when the message states no
   * mode, and - through {@link Trip} itself - to the operator and submode of {@link #route} when
   * none is resolved. The headsign is not creation data - see {@link #tripHeadsign()}.
   */
  private void applyTripDescription(TripBuilder builder) {
    builder.withMode(mode != null ? mode : route.getMode());
    if (netexSubmode != null) {
      builder.withNetexSubmode(netexSubmode);
    }
    if (tripShortName != null) {
      builder.withShortName(tripShortName);
    }
    if (operator != null) {
