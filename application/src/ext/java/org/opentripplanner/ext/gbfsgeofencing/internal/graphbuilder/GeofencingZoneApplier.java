...
/**
 * Applies geofencing zones to street edges by adding rental restriction extensions.
 * <p>
 * This is a standalone implementation for the sandbox feature, avoiding dependencies
 * on the vehicle rental updater package.
 * <p>
 * Even though the data is kept on the vertex this applier operates mostly on edges which then
 * delegate to the vertices. This is because we want to drop the vehicle outside the geofencing
 * zone rather than on the first vertex inside of it. To make this work we need to know which
 * are the edges that cross the border.
 */
class GeofencingZoneApplier {

  private final Function<Envelope, Collection<Edge>> findEdges;

  GeofencingZoneApplier(Function<Envelope, Collection<Edge>> findEdges) {
   ...
   *
   * @return Map of modified edges to their applied extensions
   */
  ...
  private Map<StreetEdge, RentalRestrictionExtension> applyExtension(
    ...
    if (geom instanceof LineString ring) {
      ...
      candidates = Set.copyOf(findEdges.apply(geom.getEnvelopeInternal()));
    }
