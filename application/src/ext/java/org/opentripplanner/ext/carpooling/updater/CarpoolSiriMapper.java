...
public class CarpoolSiriMapper {
   ...
   * Cancelled calls are skipped. Only the first element of each remaining call's capacities
   * list is inspected; additional entries are ignored. Uses the value from the first
   * non-cancelled call that has it. Logs a warning if non-cancelled calls report different
   * capacity values. Returns {@link CarpoolTrip#DEFAULT_TOTAL_CAPACITY} if no non-cancelled
   * call has capacity data or if the value is invalid.
   */
  private int extractTotalCapacity(String tripId, List<EstimatedCall> calls) {
    ...
    for (int i = 0; i < calls.size(); i++) {
      ...
      var capacities = call.getExpectedDepartureCapacities();
