...
public class DefaultCarpoolingService implements CarpoolingService {
   ...
   * and leave. Application-scoped and shared with the ingest-time trip resolver, so per-vertex
   * reachability verdicts are computed once per curb and reused across requests — the verdict
   * depends only on the static street graph.
   */
