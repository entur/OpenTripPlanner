...
public class GtfsRealTimeTripUpdateAdapter implements GtfsTripUpdateAdapter {
  ...
  private Result<UpdateSuccess, UpdateError> handleCanceledTrip(
    ...
    // For DIFFERENTIAL updates, check if there is a previously added trip
    if (incrementality != FULL_DATASET) {
      final TripPattern addedPattern = snapshotManager.getNewTripPatternForModifiedTrip(
        tripUpdate.tripId(),
        tripUpdate.serviceDate()
      );
      if (addedPattern != null) {
        ...
        if (timetable != null) {
          ...
          if (tripTimes != null && tripTimes.getRealTimeState() == RealTimeState.ADDED) {
            // Cancel the previously added trip
            ...
            snapshotManager.updateBuffer(
              new RealTimeTripUpdate(addedPattern, builder.build(), tripUpdate.serviceDate())
            );
    ...
    // Cancel/delete the scheduled trip
    final TripPattern pattern = getPatternForTripId(tripUpdate.tripId());
    if (pattern == null) {
