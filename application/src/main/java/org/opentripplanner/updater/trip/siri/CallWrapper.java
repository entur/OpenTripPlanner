...
public interface CallWrapper {
  ...
  /**
   * Map this call to a drop-off type for the stop arrival, relative to the scheduled value.
   *
   ...
   */
  Optional<PickDrop> mapDropOffType(PickDrop plannedValue);

  /**
   * Map this call to a pick-up type for the stop departure, relative to the scheduled value.
   *
   * @param plannedValue the current pick drop value on a stopTime
   * @return the mapped {@link PickDrop} type, empty if routability is not changed
   */
  Optional<PickDrop> mapPickUpType(PickDrop plannedValue);
  ...
  final class RecordedCallWrapper implements CallWrapper {
    ...
    @Override
    public Optional<PickDrop> mapDropOffType(PickDrop plannedValue) {
      return CallPickDropMapper.mapDropOffType(plannedValue, isCancellation(), null, null);
    }

    @Override
    public Optional<PickDrop> mapPickUpType(PickDrop plannedValue) {
      return CallPickDropMapper.mapPickUpType(plannedValue, isCancellation(), null, null);
    }
