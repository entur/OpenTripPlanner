...
public final class PickDropChange {
  ...
  /**
   * Whether the message cancels this call end. SIRI states that either with the {@code Cancellation}
   * element of the call - which cancels both of its ends - or with an arrival/departure status of
   * {@code cancelled}, which cancels one end only. Both forms are normalized away at construction,
   * so a caller cannot tell them apart, and a recorded call reports no status at all.
   * <p>
   * A cancelled end carries no {@link #intent()}: what it does to boarding depends on the scheduled
   * value, which {@link #applyTo(PickDrop)} resolves. A caller that resolves the value itself has to
   * ask this first, because a cancellation and a pass-thru call both end up as
   * {@link PickDrop#CANCELLED} and only the cancellation leaves an already non-routable value alone.
   */
