...
/**
 * An update to a previously added real-time trip: the same trip is sent again as ADD_NEW_TRIP after
 * it has already been integrated in the transit model (a subsequent update to an extra journey).
 * <p>
 * The update owns its own application through {@link #apply()}: the existing trip and pattern are
 * reused verbatim, only the trip times are rebuilt from the baseline times and the incoming call
 * data. {@link org.opentripplanner.updater.trip.AddedTripUpdater} drives it.
 */
