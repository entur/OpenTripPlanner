 ...
 * point of {@link org.opentripplanner.updater.trip.ExistingTripResolver} the caller picks - not by
 * a runtime check - and each validated against the invariants of its own use case:
 * <ul>
 *   <li>{@link ResolvedScheduledTripUpdate} - the trip keeps running its scheduled pattern and only
 *       its times and minor pattern details change</li>
 *   <li>{@link ResolvedTripModification} - the trip is rerouted on a new stop pattern for the
 *       service date</li>
 * </ul>
 */
public abstract sealed class ResolvedExistingTrip
  permits ResolvedScheduledTripUpdate, ResolvedTripModification {
  ...
  protected ResolvedExistingTrip(
