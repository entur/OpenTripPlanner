...
/**
 * Cache key for the per-request transfer index. The incoming {@link RouteRequest} is
 * mapped to a {@link StreetSearchRequest} and then snapped onto {@link BucketGrid} via
 * {@link TransferRequestBucketer} to reduce the number of distinct keys. Equality and
 * hashing are delegated to {@link StreetRelevantOptions}, which extracts only the
 * parameters that influence transfer pre-computation.
 */
class RaptorRequestTransferCacheKey {
  ...
  public RaptorRequestTransferCacheKey(
    ...
    var mapped = StreetSearchRequestMapper.mapToTransferRequest(request).build();
    this.request = TransferRequestBucketer.bucket(mapped);
