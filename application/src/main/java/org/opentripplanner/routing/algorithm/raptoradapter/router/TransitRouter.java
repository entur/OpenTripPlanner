...
public class TransitRouter {
  ...
  /**
   * Returns {@code true} when the request has no way to reach transit because neither an access
   * nor an egress street mode was requested. In that case the raptor search would fail in
   * {@link #verifyAccessEgress} anyway, so we can skip the expensive per-request transit-data
   * build. This guards against API callers that supply a direct street mode without explicitly
   * disabling transit (e.g. Transmodel queries like {@code modes: { directMode: foot }} that omit
  ...
  private static boolean cannotReachTransit(RouteRequest request) {
    ...
    return (
      journey.access().mode() == StreetMode.NOT_SET && journey.egress().mode() == StreetMode.NOT_SET
    );
  }
}
