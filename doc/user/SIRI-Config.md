...
| shadowComparison                |    `boolean`    | Run the legacy and unified adapters in parallel, comparing their outputs. The legacy adapter writes to the snapshot; the unified adapter is shadow (read-only). Mismatches are logged as warnings.            | *Optional* | `false`       |  2.9  |
...
| useNewUpdaterImplementation     |    `boolean`    | Use the new unified trip update implementation. When true, uses the new format-independent implementation shared by SIRI-ET and GTFS-RT. When false (default), uses the legacy SiriRealTimeTripUpdateAdapter. | *Optional* | `false`       |  2.9  |
