...
public interface RaptorTripSchedule {
  ...
  /// the last stop in the pattern. We do not know the alight-stop, so it is impossible to calculate
  ...
  ///   pattern), `relativeTravelDuration(boardAtI) - relativeTravelDuration(boardAtJ)` must equal
  ///   the actual transit duration from stop `i` to stop `j`. If the board time at position 3 is
  ///   10:00 and at the next stop is 10:05, then the value at position 3 is larger by 5*60s = 300
